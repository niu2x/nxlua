...
// IMGUI_USER_CONFIG "myfilename.h" from your build system')
    ...
    // Version
    // (Integer encoded as XYYZZ for use in #if preprocessor conditionals. Work
    // in progress versions typically starts at XYY99 then bounce up to XYY00,
    ...
    // Windows) IMGUI_API is used for core imgui functions, IMGUI_IMPL_API is
    // used for the default backends files (imgui_impl_xxx.h) Using dear imgui
    // via a shared library is not recommended, because we don't guarantee
    // backward nor forward ABI compatibility (also function call overhead, as
    ...
    // Helper Macros - IM_FMTARGS, IM_FMTLIST: Apply printf-style warnings to
...
typedef int ImGuiWindowFlags; // -> enum ImGuiWindowFlags_     // Flags: for
    ...
    // ImTexture: user data for renderer backend to identify a texture
    ...
    // - This can be whatever to you want it to be! read the FAQ about
...
typedef void*
    ...
    // - To use 32-bit indices: override with '#define ImDrawIdx unsigned int'
...
typedef unsigned int
    ImWchar32; // A single decoded U32 character/code point. We encode them as
               // multi bytes UTF-8 when used in strings.
                             ...
                             // imconfig.h with '#define IMGUI_USE_WCHAR32' to
...
IMGUI_API bool IsMouseTripleClicked(ImGuiMouseButton
        button); // did mouse button triple-clicked? (note that a triple-click
                 // will also report IsMouseClicked() == true)
...
enum ImGuiDragDropFlags_ {
    ...
    // Standard Drag and Drop payload types. You can define you own payload
    // types using short strings. Types starting with '_' are defined by Dear
    // ImGui.
...
struct ImGuiIO {
    ...
    IMGUI_API void AddFocusEvent(
        bool focused); // Notifies Dear ImGui when hosting platform windows lose
                       // or gain input focus
    IMGUI_API void
...
struct ImColor {
    ...
    // series of drawing commands. The user provides a renderer for ImDrawData
    // which essentially contains an array of ImDrawList.
    //-----------------------------------------------------------------------------

    // The maximum line width to bake anti-aliased textures for. Build atlas
    // with ImFontAtlasFlags_NoBakedLines to disable baking.
    #ifndef IM_DRAWLIST_TEX_LINES_WIDTH_MAX
        #define IM_DRAWLIST_TEX_LINES_WIDTH_MAX (63)
    #endif

    // ImDrawCallback: Draw callbacks for advanced uses [configurable type:
    // override in imconfig.h] NB: You most likely do NOT need to use draw
    // callbacks just to create your own widget or customized UI rendering, you
    // can poke into the draw list for that! Draw callback may be useful for
    // example to:
    ...
    // (cmd.UserCallback != NULL) { cmd.UserCallback(parent_list, cmd); } else {
    // RenderTriangles() }' If you want to override the signature of
    // ImDrawCallback, you can simply use e.g. '#define ImDrawCallback
...
typedef void (*ImDrawCallback)(
    ...
    // altered the render state and you want it to be restored. It is not done
    // by default because they are many perfectly useful way of altering render
    // state for imgui contents (e.g. changing shader/blending settings before
    // an Image call).
...
struct ImFontAtlas {
    ...
    #ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
    typedef ImFontAtlasCustomRect
        CustomRect; // OBSOLETED in 1.72+
