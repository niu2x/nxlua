    ...
    switch (t->tag) {
        ...
        case TOpenCall: {
            if (g != NULL) /* inside a grammar? */
                ...
                luaL_error(
                    L, "rule '%s' used outside a grammar", val2str(L, -1));
            }
            break;
        }
        case TSeq:
...
static TTree* getpatt(lua_State* L, int idx, int* len)
{
    ...
    switch (lua_type(L, idx)) {
        ...
        case LUA_TBOOLEAN: {
            tree = (lua_toboolean(L, idx) ? newleaf(L, TTrue)
                                          : newleaf(L, TFalse));
            break;
        }
        case LUA_TTABLE: {
...
static void getfirstrule(lua_State* L, int arg, int postab)
{
    ...
    if (!testpattern(L, -1)) { /* initial rule not a pattern? */
        if (lua_isnil(L, -1))
            ...
            luaL_error(
                L, "initial rule '%s' is not a pattern", lua_tostring(L, -2));
    }
    ...
    for (i = npassed - 1; i >= 0; i--) { /* search for a repetition */
        for (j = i - 1; j >= 0; j--) {
            if (passed[i] == passed[j]) {
                ...
                return luaL_error(
                    L, "rule '%s' may be left recursive", val2str(L, -1));
            }
        }
    }
...
*/
static int verifyrule(
    lua_State* L, TTree* tree, int* passed, int npassed, int nb)
{
tailcall:
    switch (tree->tag) {
        case TChar:
...
static struct luaL_Reg pattreg[] = { { "ptree", lp_printtree },
    { "pcode", lp_printcode }, { "match", lp_match }, { "B", lp_behind },
    { "V", lp_V }, { "C", lp_simplecapture }, { "Cc", lp_constcapture },
    { "Cmt", lp_matchtime }, { "Cb", lp_backref }, { "Carg", lp_argcapture },
    { "Cp", lp_poscapture }, { "Cs", lp_substcapture },
    { "Ct", lp_tablecapture }, { "Cf", lp_foldcapture },
    { "Cg", lp_groupcapture }, { "P", lp_P }, { "S", lp_set },
    { "R", lp_range }, { "locale", lp_locale }, { "version", lp_version },
    { "setmaxstack", lp_setmax }, { "type", lp_type }, { NULL, NULL } };
