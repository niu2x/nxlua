...
*/
static void codegen(
    CompileState* compst, TTree* tree, int opt, int tt, const Charset* fl);
...
**
*/
static int addinstcap(
    CompileState* compst, Opcode op, int cap, int key, int aux)
{
...
*/
static void codechoice(
    CompileState* compst, TTree* p1, TTree* p2, int opt, const Charset* fl)
{
...
** CloseCapture.
*/
static void codecapture(
    CompileState* compst, TTree* tree, int tt, const Charset* fl)
{
...
*/
static void coderep(
    CompileState* compst, TTree* tree, int opt, const Charset* fl)
{
    Charset st;
...
*/
static int codeseq1(
    CompileState* compst, TTree* p1, TTree* p2, int tt, const Charset* fl)
{
    if (needfollow(p1)) {
        Charset fl1;
...
tailcall:
    switch (tree->tag) {
        ...
        case TSeq: {
            tt = codeseq1(
                compst, sib1(tree), sib2(tree), tt, fl); /* code 'p1' */
    ...
    redo:
        switch (code[i].i.code) {
            ...
            case ITestAny: { /* instructions with labels */
                jumptothere(
                    compst, i, finallabel(code, i)); /* optimize label */
                break;
            }
            case IJmp: {
                ...
                switch (code[ft].i.code) { /* jumping to what? */
                    ...
                    case IFail:
                    case IFailTwice:
                    case IEnd: { /* instructions with unconditional implicit
                                    jumps */
                        code[i] = code[ft]; /* jump becomes that instruction */
                        code[i + 1].i.code
                            = IAny; /* 'no-op' for target position */
                        break;
                    }
                    case ICommit:
                    case IPartialCommit:
                    case IBackCommit: { /* inst. with unconditional explicit
                                           jumps */
                        int fft = finallabel(code, ft);
                        code[i]
                            = code[ft]; /* jump becomes that instruction... */
                        jumptothere(
                            compst, i, fft); /* but must correct its offset */
