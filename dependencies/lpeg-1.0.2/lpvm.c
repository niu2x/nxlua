...
*/
static Capture* growcap(
    lua_State* L, Capture* capture, int* capsize, int captop, int n, int ptop)
{
    if (*capsize - captop > n)
        ...
        switch ((Opcode)p->i.code) {
            ...
            fail : { /* pattern failed: try to backtrack */
                ...
                if (ndyncap > 0) /* is there matchtime captures? */
                    ndyncap
                        -= removedyncap(L, capture, stack->caplevel, captop);
                captop = stack->caplevel;
                p = stack->p;
#if defined(DEBUG)
                printf("**FAIL**\n");
            ...
            case ICloseRunTime: {
                ...
                cs.ptop = ptop;
                n = runtimecap(
                    &cs, capture + captop, s, &rem); /* call function */
                ...
                if (n == 0) /* no new captures? */
                    ...
                    capture
                        = growcap(L, capture, &capsize, captop, n + 1, ptop);
